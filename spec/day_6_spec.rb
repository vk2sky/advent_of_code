...
class LightArray
  ...
  def initialize
    @lights = Array.new(ROWS) { Array.new(COLUMNS, 0) }
  end
  ...
  def number_off
    @lights.flatten.count(0)
  end

  def command(cmd)
    if cmd =~ /(\D*) (\d*),(\d*) through (\d*),(\d*)/
      operation, from_row, from_column, to_row, to_column = $1, $2, $3, $4, $5
      ...
      do_command(command, from_row, from_column, to_row, to_column)
  ...
  def command_2(cmd)
    # part 2 of the puzzle interprets the on/off/toggle commands differently
    ...
    if cmd =~ /(\D*) (\d*),(\d*) through (\d*),(\d*)/
      ...
      command =
          case operation
            when /turn on/  then :increase_brightness
            when /turn off/ then :decrease_brightness
            when /toggle/   then :increase_brightness_by_2
            else raise "unexpected operation, '#{operation}'"
          end
  ...
  def brightness
    @lights.flatten.inject(0) { |total, light| total + (light == 0 ? 0 : light) }
  end

  private

  def do_command(command, from_row, from_column, to_row, to_column)
    from_row    = from_row.to_i
    from_column = from_column.to_i
    to_row      = to_row.to_i
    to_column   = to_column.to_i

    (from_row..to_row).each do |row|
      (from_column..to_column).each do |column|
        current_light_value = @lights[row][column]

        case command
          # part 1 commands
          when :on     then @lights[row][column] = 1
          when :off    then @lights[row][column] = 0
          when :toggle then @lights[row][column] = (current_light_value == 1 ? 0 : 1)

          # part 2 commands
          when :increase_brightness
            @lights[row][column] += 1

          when :decrease_brightness
            @lights[row][column] -= 1 if current_light_value > 0

          when :increase_brightness_by_2
            @lights[row][column] += 2
...
describe 'Day 6' do
  before(:each) { @lights = LightArray.new }


  context 'part 1' do
    specify 'new light array is 1 000 000 lights, all off' do
      expect(@lights.number).to eq 1_000_000
      expect(@lights.number_on).to eq 0
      expect(@lights.number_off).to eq 1_000_000
    end

    it '"turn on 0,0 through 999,999" turns on all lights' do
      @lights.command('turn on 0,0 through 999,999')
      expect(@lights.number_on).to eq 1_000_000
    end

    it '"toggle 0,0 through 999,0" changes the states of the first column of lights' do
      @lights.command('toggle 0,0 through 999,0')
      expect(@lights.number_on).to eq 1_000

      1_000.times do |row|
        expect(@lights[row][0]).to eq 1
        expect(@lights[row][1..999].all? { |light| light.zero? } ).to be_truthy
      end
    end

    it '"turn off 499,499 through 500,500" turns off only the middle 4 lights' do
      @lights.command('turn on 0,0 through 999,999')
      @lights.command('turn off 499,499 through 500,500')
      expect(@lights.number_off).to eq 4
    end

    it 'puzzle input turns on 569 999 lights' do
      $instructions.each { |instruction| @lights.command(instruction) }
      expect(@lights.number_on).to eq 569_999
    end
  end


  context 'part 2' do
    it '"turn on 0,0 through 0,0" turns increases brightness by 1' do
      @lights.command_2('turn on 0,0 through 0,0')
      expect(@lights.brightness).to eq 1
    end

    it '"toggle 0,0 through 999,999" turns increases brightness by 2 000 000' do
      @lights.command_2('toggle 0,0 through 999,999')
      expect(@lights.brightness).to eq 2_000_000
    end

    it 'puzzle input increases light brightness by 17 836 115' do
      $instructions.each { |instruction| @lights.command_2(instruction) }
      expect(@lights.brightness).to eq 17_836_115
    end
  end
end